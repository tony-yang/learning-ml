...
maildir = os.path.join('194_enron', 'maildir')
email_count_from_poi_to_person = {}
poi_email_address = poi_emails()

for dirname, subdirs, filenames in os.walk(maildir):
    if '/inbox' in dirname:
        inbox_owner = dirname.split('/')[2]
        if inbox_owner not in email_count_from_poi_to_person:
            email_count_from_poi_to_person[inbox_owner] = 0

        for filename in filenames:
            mail_path = os.path.join(dirname, filename)
            print('mailpath = {}'.format(mail_path))
            if os.path.exists(mail_path):
                with open(mail_path, 'r', errors='ignore') as fh:
                    email_content = fh.read()
                    content = email_content.split('From:')
                    #print('content = {}'.format(content))
                    sender = content[1].split('\n')[0].strip()
                    if sender in poi_email_address:
                        email_count_from_poi_to_person[inbox_owner] += 1

print('email count from poi to person = {}'.format(email_count_from_poi_to_person))
